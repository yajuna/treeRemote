...
\section{To do (March)}
\begin{enumerate}
\item get heat program running in Python3 (eliminate all syntax error)
\item find appropriate parameters for testing
\item think of ways to validate computation (analytical solution, similar and simpler problem)
...
For simplicity of discussion, we assume $\rho,\ c, k$ are all constants for now. Then the constant coefficient equation \eqref{heat} simplifies to 
\begin{equation}
...
With centered FD discretization in time and space, we have
...
Here $T^n_{i,j} \approx T(r_i,\phi_j,t_n)$ denotes the numerical solution at the point $(r_i,\phi_j,t_n)$. Then the above {\it continuous differential equation} becomes the following {\it discrete difference equation}:
\begin{align}
...
\end{equation}
where we obtain $T^1_{i,j}$ each time step with $T$ values in several locations from previous step. 
...
\end{center}
Image one more axis normal to the paper, pointing outwards above the blue dot. That would be $T^1_{i,j}$. 

%For future: tikz image: 
%
