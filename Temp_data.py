...
def temp(m, t0):
    ...
    #  z is the length between data points on y. We are solving the equation z*m = size(y)
    #  which is saying (length between data points)*(total # of data points) = (last index in y)
    z = np.size(y) / m
    #  print("z = " + str(z))
...
def tempArray(t1):
    #  needed to have the +1 or the array size wouldn't match
