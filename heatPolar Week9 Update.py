...
N_steps = 200
radius = 0.09
c = 2.7148*10**(-7) # = k/(rho*c) in equation (2) of Potter and Anredsen paper #changed to match potter anderson
dphi = 2*np.pi/N_phi
dphi2 = dphi * dphi
dr = radius/Nr
dr2 = dr * dr
...
if (1 - 4*dt*c/dr2 - 4*dt*c/(dr2*dphi2))**2<1: # CFL condition.
    ...
    print(str((1 - 4*dt*c/dr2 - 4*dt*c/(dr2*dphi2))**2) + "> 1")
    dt = dt*0.0001
...
for t in range(1, N_steps):
    for i in range(0, Nr-1):
        ...
        k2 = c*dt/(2*ri*dr)
        k3 = c*dt/(dphi2*ri**2)
        k4 = c/(0.46*dr) # Changed to match potter anderson
        #En = 0 #uncomment this to make source terms always 0.
        En = i == (Nr-2)
#        print(k2,k3,i)
        for j in range(0, N_phi-1):
            ...
            - k4*sourceTerms(En, dphi*j)        # Need to use sourceTerms function from sourceTerms.py
